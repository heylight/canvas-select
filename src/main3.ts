...
class CanvasSelect {
  ...
  get createShape() {
    return this.dataset.find((x: Rect | Polygon) => x.creating);
  }
  ...
  init() {
    ...
    this.canvas.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      if (e.buttons === 2) {
        ...
        this.ctrlIndex = ctrls.findIndex((coor) => this.isPointInCircle([e.offsetX, e.offsetY], coor));
        if (this.ctrlIndex > -1) {
        ...
        if (this.activeShape?.type === 2 && (this.activeShape as Polygon).creating) {
        ...
        const targetShape = this.dataset.reduceRight((target, shape, i) => {
          if (!target) {
            if (
              (shape.type === 3 && this.isPointInCircle([e.offsetX, e.offsetY], shape.coor as Point))
              || this.isPointInArea([e.offsetX, e.offsetY], shape)
            ) {
              targetShapeIndex = i;
              target = shape;
              target.active = true;
        ...
        if (targetShape) {
          ...
          this.dataset.forEach((sp) => { sp.active = targetShape === sp; });
          ...
          if (targetShape.type === 3) {
            ...
            (targetShape as Rect | Polygon).coor.forEach((pt: Point) => {
          ...
          if (this.createType === 1) {
            ...
            newShape = new Dot([offsetX - this.originX / this.scale, offsetY - this.originY / this.scale], this.dataset.length);
          }
    ...
    this.canvas.addEventListener('mousemove', (e: MouseEvent) => {
      ...
      if (e.buttons === 2 && e.which === 3) {
        ...
        if (this.activeShape && this.isInContent(e)) {
          if ((this.activeShape as Rect | Polygon).dragging) {
            const coor: Point[] = [];
          ...
          } else if ((this.activeShape as Rect | Polygon).creating && this.createType === 1) {
          ...
          if (this.activeShape.type === 1) {
            const [[x0, y0], [x1, y1]] = (this.activeShape as Rect | Polygon).coor;
            let coor: Point[] = [];
        ...
        const ctrls = (this.activeShape as Rect | Polygon)?.ctrlsData || [];
        const isHoverCtrl = ctrls.find((coor) => this.isPointInCircle([e.offsetX, e.offsetY], coor));
        const isHoverShape = this.dataset.find((shape) => {
          if (shape.type === 3) {
            return this.isPointInCircle([e.offsetX, e.offsetY], shape.coor as Point);
          }
          return this.isPointInArea([e.offsetX, e.offsetY], shape);
        });
    ...
    this.canvas.addEventListener('mouseup', () => {
      ...
      if (this.createShape?.type === 1) {
        const [[x0, y0], [x1, y1]] = (this.createShape as Rect).coor;
        ...
        this.dataset.forEach((x: Rect | Polygon) => { if (x.type !== 3) x.creating = false; });
        this.update();
        this.emit('update');
      } else if (this.activeShape?.type === 3) {
        this.update();
        this.emit('update');
      }
    });
    this.canvas.addEventListener('dblclick', () => {
      ...
      if (this.createShape?.type === 2) {
        if (this.createShape.coor.length > 2) {
          this.emit('add', this.createShape);
          (this.createShape as Polygon).creating = false;
          this.update();
        }
      }
    });
      ...
      if (Object.prototype.toString.call(item).indexOf('Object') > -1) {
        ...
        if (typeof type === 'number' && type > 0) {
          if (type === 1 && coor.length === 2) {
            shape = new Rect(coor as any, index);
          }
          if (type === 2 && coor.length > 2) {
            shape = new Polygon(coor as any, index);
          }
          if (type === 3) {
            shape = new Dot(coor as any, index);
          }
   ...
   * @param area 目标区域
   */
  isPointInArea(point: Point, shape: (Rect | Polygon | Dot)) {
    ...
    if (shape.type === 1) {
      const [[x0, y0], [x1, y1]] = (shape as Rect).coor;
      ...
      (shape as Polygon).coor.forEach((pt, i) => {
   ...
   * @param area 目标区域
   */
  isPointInCircle(point: number[], area: number[]) {
    this.offlineCtx.save();
    const [x0, y0] = area;
    const x = Math.round(x0 * this.scale);
    const y = Math.round(y0 * this.scale);
  ...
  drawPolygon(shape: Polygon) {
    ...
    if (shape.coor.length > 2 && !shape.creating) {
      this.ctx.closePath();
    }
    this.ctx.fill();
    this.ctx.stroke();
   ...
   */
  drawDot(shape: Dot) {
    const [x0, y0] = shape.coor;
    const x = x0 * this.scale;
    const y = y0 * this.scale;
    this.ctx.save();
    this.ctx.fillStyle = this.fillStyle;
  ...
  update() {
    ...
    if (this.activeShape && [1, 2].includes(this.activeShape.type)) {
      (this.activeShape as Rect | Polygon).ctrlsData.forEach((el) => {
        this.drawCtrls(el);
      });
    }
    this.ctx.restore();
