...
class Rect implements BaseShape {
  index: number
  label: string = ''
  type: number = 1
...
class Polygon implements BaseShape {
  index: number
  label: string = ''
  type: number = 2
  active: boolean = false
...
class CanvasSelect {
  ...
  constructor(el: HTMLCanvasElement | string) {
    ...
    if (Object.prototype.toString.call(container).includes('HTMLCanvasElement')) {
      ...
      console.error(el, 'HTMLCanvasElement is required!')
    }
  }
  setData(data: Array<Rect | Polygon>) {
    data.forEach((item, index) => {
      const { label, type, coor, uuid } = item
  ...
  init() {
    ...
    this.canvas.addEventListener('mousedown', (e: MouseEvent) => {
      if (this.activeShape) {
        ...
        if (this.ctrlIndex > -1) {
          let [x0, y0] = ctrls[this.ctrlIndex]
          this.remmber = [[e.offsetX - x0, e.offsetY - y0]]
          return
        }
      }
      this.dataset.reverse()
      let targetShape = this.dataset.find(shape => this.isPointInArea([e.offsetX, e.offsetY], shape))
      this.dataset.reverse()
      ...
      if (this.createShape) {
        ...
        if (this.createShape.type === 2) {
          if (!(this.createShape as Polygon).finish) {
            let [x, y] = this.createShape.coor[this.createShape.coor.length - 1]
        ...
        let targetShapeIndex = this.dataset.findIndex(x => x === targetShape)
    ...
    this.canvas.addEventListener('mousemove', (e: MouseEvent) => {
      let hoverShape = this.dataset.find(shape => this.isPointInArea([e.offsetX, e.offsetY], shape))
    ...
    document.body.addEventListener('keyup', (e: KeyboardEvent) => {
      if (this.createShape && this.createShape.type === 2) {
        if (e.key === 'Escape') {
          ...
          this.createShape.coor.length > 1 ? this.createShape.coor.pop() : this.dataset.pop()
        }
        this.update()
      }
    })
